...
async function getEditableClass(id: string): Promise<ClassFormValues> {
  await requireAdmin();
...
export default async function EditClassPage({
  ...
  const initialValues = await getEditableClass(id);
  ...
  return (
    <section className="admin-page admin-page--narrow">
      ...
      <h1>Edit class</h1>
      <p className="intro">Update the class details without changing its scheduled/canceled status.</p>
      <ClassForm action={action} initialValues={initialValues} submitLabel="Save class" />
